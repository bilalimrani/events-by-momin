...
export default function ServiceAreas() {
  return (
    <section id="areas" className="section-padding bg-cream-100 relative overflow-hidden">
      ...
      <div
        aria-hidden="true"
        className="absolute top-10 right-[-6rem] w-80 h-80 bg-gold-400/8 rounded-full blur-3xl"
      />
      <motion.svg
        aria-hidden="true"
        animate={{ rotate: 360 }}
        transition={{ duration: 40, repeat: Infinity, ease: "linear" }}
        className="hidden lg:block absolute -bottom-16 -left-16 w-64 h-64 text-gold-400/10"
        viewBox="0 0 100 100"
        fill="none"
      >
        ...
        {cityDetails.map((city, i) => (
          <motion.div
            ...
            className={`group relative overflow-hidden rounded-2xl p-6 pl-7 text-left shadow-sm border transition-all duration-300 hover:shadow-xl ${
              city.home
                ? "bg-gradient-royal bg-300% animate-gradientMove border-transparent shadow-lg shadow-black/40 sm:col-span-2"
                : "bg-maroon-800 border-gold-400/20 hover:border-gold-400/50"
            }`}
          >
            <span
              aria-hidden="true"
              className={`absolute left-0 top-0 bottom-0 w-1.5 ${
                city.home ? "bg-gold-400" : "bg-gradient-to-b from-gold-400 to-gold-600"
              }`}
            />
            ...
            <div className="flex items-start gap-3">
              <span
                className={`mt-0.5 flex-shrink-0 w-9 h-9 rounded-full flex items-center justify-center shadow-md transition-transform duration-300 group-hover:scale-110 ${
                  city.home ? "bg-gold-400 text-black" : "bg-gradient-to-br from-gold-600 to-gold-400 text-black"
                }`}
              >
              ...
              <div>
                <h3
                  className={`font-display text-lg font-bold mb-1.5 ${
                    city.home ? "text-cream-50" : "text-gold-300"
                  }`}
                >
                  <a
                    href={city.href}
                    className={`hover:underline ${city.home ? "text-cream-50" : "text-gold-300"}`}
                  >
                    {city.name}
                  </a>
                </h3>
                <p className={`text-sm leading-relaxed ${city.home ? "text-cream-50/85" : "text-cream-50/70"}`}>
                  {city.blurb}
                </p>
                <a
                  href={city.href}
                  className={`inline-block mt-2 text-xs font-semibold underline ${city.home ? "text-gold-300 hover:text-gold-200" : "text-gold-400 hover:text-gold-300"}`}
                >
                  View details →
                </a>
              </div>
      ...
      <div className="max-w-4xl mx-auto mt-8 flex flex-wrap justify-center gap-3 relative z-10">
        {siteConfig.serviceAreas.map((area, i) => (
          <motion.span
            ...
            whileHover={{ scale: 1.07, backgroundColor: "#c9a227", color: "#000000" }}
            className="inline-flex items-center gap-1.5 px-5 py-2.5 rounded-full border-2 border-gold-400/50 text-gold-400 text-sm font-semibold bg-maroon-800 shadow-sm cursor-default transition-colors"
          >
      ...
      <p className="max-w-2xl mx-auto mt-10 text-center text-sm text-charcoal/60 relative z-10">
        ...
        and corporate events — <a href="#contact" className="text-gold-400 font-semibold underline">get in touch</a> and let&apos;s talk about your event.
      </p>
    </section>
  );
}
